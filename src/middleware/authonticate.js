...
//    if we receive  the bearer token in the header

const bearerToken = req?.headers?.authorization
// if not receive or token is not a bearer then we will throw an error
if(!bearerToken || !bearerToken.startsWith('Bearer ')){
    return res.status(404).json({message:"Please provide a valid token",status:"Failed"})
}
